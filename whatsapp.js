...
// ============================================
// (Dependencia eliminada: outlook-monitor ya no se usa)
...
async function sendEmailViaN8N(emailPayload) {
    return new Promise((resolve) => {
        try {
            const urlObj = new URL(N8N_EMAIL_WEBHOOK);
            ...
            const req = https.request(options, (res) => {
                ...
                res.on('end', () => {
                    if (res.statusCode >= 200 && res.statusCode < 300) {
                        console.log(`✅ Email enviado a N8N Webhook (${res.statusCode})`);
                        resolve({ success: true, response: responseData });
                    } else {
                        console.error(`❌ Error enviando a N8N (${res.statusCode}):`, responseData);
                        resolve({ success: false, error: `Status ${res.statusCode}: ${responseData}` });
                    }
                });
            });

            req.on('error', (error) => {
                console.error('❌ Error de red al enviar a N8N:', error.message);
                resolve({ success: false, error: error.message });
            });

            req.on('timeout', () => {
                req.destroy();
                resolve({ success: false, error: 'Timeout connecting to N8N' });
            });
            ...
            console.error('❌ Excepción al enviar a N8N:', e.message);
...
async function notifyNewEmail(emailData, source) {
    try {
        ...
        return {
            ...
            whatsappResults,
            emailResults,
            formattedMessage: messageWithGestor || message
        };
    } catch (error) {
